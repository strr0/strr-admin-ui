import type { CustomRoute, RouteLocationNormalizedLoaded, RouteRecordRaw } from 'vue-router';
import { $t } from '@/locales';
import { useSvgIcon } from '@/hooks/modules/icon';
 ...
 */
export function getGlobalMenusByAuthRoutes(routes: CustomRoute[]) {
 ...
 * @param route
 */
function getGlobalMenuByBaseRoute(route: RouteLocationNormalizedLoaded | CustomRoute) {
 ...
 * @param routes
 */
export function isRouteExistByRoutePath(routePath: string, routes: CustomRoute[]) {
  return routes.some(route => recursiveGetIsRouteExistByRoutePath(route, routePath));
}
 ...
 * @param routeName
 */
function recursiveGetIsRouteExistByRoutePath(route: CustomRoute, routePath: string) {
  ...
  if (route.children && route.children.length) {
    isExist = route.children.some(item => recursiveGetIsRouteExistByRoutePath(item, routePath));
  }
