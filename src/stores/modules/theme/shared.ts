...
import { addColorAlpha, getColorPalette, getPaletteColorByNumber, getRgb } from '@/settings/color';
import { overrideThemeSettings, themeSettings } from '@/settings/theme/settings';
import { themeVars } from '@/settings/theme/vars';
...
export function initThemeSettings() {
  ...
  // if it is development mode, the theme settings will not be cached, by update `themeSettings` in `src/settings/theme/settings.ts` to update theme settings
  if (!isProd) return themeSettings;

  // if it is production mode, the theme settings will be cached in localStorage
  // if want to update theme settings when publish new version, please update `overrideThemeSettings` in `src/settings/theme/settings.ts`
