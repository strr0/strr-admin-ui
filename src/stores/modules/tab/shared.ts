 ...
 *
 * @param router
 * @param homeRoutePath routeHome in useRouteStore
 */
export function getDefaultHomeTab(router: Router, homeRoutePath: string) {
  const routes = router.getRoutes();
  const homeRoute = routes.find(route => route.path === homeRoutePath);
  if (homeRoute) {
    return getTabByRoute(homeRoute);
  }

  const i18nLabel = $t('route.home');
  const homeTab: App.Global.Tab = {
    id: '/home',
    label: i18nLabel || 'home',
    routeKey: 'home',
    routePath: '/home',
    fullPath: '/home'
  };
